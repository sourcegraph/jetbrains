...
val pnpmBinary = System.getenv("PNPM_BINARY") ?: "pnpm"
...
tasks {
  ...
  fun buildCodeSearch(): File? {
    ...
    exec {
      ...
      commandLine(pnpmBinary, "install", "--frozen-lockfile")
    }
    exec {
      workingDir(sourcegraphDir.toString())
      commandLine(pnpmBinary, "generate")
    }
    val jetbrainsDir = sourcegraphDir.resolve("client").resolve("jetbrains")
    exec {
      commandLine(pnpmBinary, "build")
  ...
  val codyCommit = "5d19f27e40192a97e29bd30d0cfba9cf9815cd95"
  ...
  val codyDirEnvironmentVariable = System.getenv("CODY_DIR")
  fun unzipCody(): File {
    if (!codyDirEnvironmentVariable.isNullOrEmpty()) {
      // "~" works fine from the terminal, however it breaks IntelliJ's run configurations
      val pathString =
          if (codyDirEnvironmentVariable.startsWith("~")) {
            System.getProperty("user.home") + codyDirEnvironmentVariable.substring(1)
          } else {
            codyDirEnvironmentVariable
          }
  ...
  fun copyAutogeneratedAgentBindings(codyDir: File): Unit {
    val outputDirectory = rootDir.resolve("src").resolve("main").resolve("kotlin").resolve("lib")
    val inputDirectory = codyDir.resolve("agent").resolve("bindings")
  ...
  fun buildCody(): File {
    ...
    if (codyDirEnvironmentVariable == null) {
      copyAutogeneratedAgentBindings(codyDir)
    }
    ...
    exec {
      workingDir(agentDir)
      commandLine(pnpmBinary, "run", "build-agent-binaries")
