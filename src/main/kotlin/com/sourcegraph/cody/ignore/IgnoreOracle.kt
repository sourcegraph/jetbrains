...
class IgnoreOracle(private val project: Project) {
  ...
  fun onIgnoreDidChange() {
    ...
    // Update editor notifications to refresh IgnoreNotificationProvider banners. We don't use a
    // listener for this
    // so that the IgnoreNotificationProvider does not have to register with each project's
    // IgnoreOracle.
  ...
  fun policyForUri(uri: String): CompletableFuture<IgnorePolicy> {
    ...
    CodyAgentService.withAgent(project) { agent ->
      val policy =
          when (agent.server.ignoreForUri(IgnoreForUriParams(uri)).get().policy) {
  ...
  /**
   * Adds a listener. Thread safe. Listener will not receive any events that are already in the
  ...
  /**
   * Removes a listener. Thread safe. If this is called during event dispatch, the listener may
   * still recieve a notification.
   */
...
/**
 * Callback for detecting and reacting to changes in the policy of which files and repositories to
 * ignore.
 */
interface IgnorePolicyListener {
  /** Notifies the listener that the ignore policy has changed. Called on a pooled thread. */
  @RequiresBackgroundThread fun onPolicyChange()
}
