...
class CodyStatusBarActionGroup : DefaultActionGroup() {
  override fun update(e: AnActionEvent) {
    ...
    deriveWarningAction(e.project!!).thenAccept { warningActions ->
  ...
  private fun deriveWarningAction(
      project: Project
  ): CompletableFuture<RateLimitErrorWarningAction?> {
    return UpgradeToCodyProNotification.isCodyProJetbrains(project).thenApply { isCodyPro ->
            ...
            autocompleteRLE != null && chatRLE != null -> {
              Triple(
                  CodyBundle.getString(
                      "status-widget.warning.autocompletion-and-chat.action-title"),
                  CodyBundle.getString("status-widget.warning.autocompletion-and-chat.content")
                      .fmt(suggestionOrExplanation),
                  CodyBundle.getString(
                      "status-widget.warning.autocompletion-and-chat.dialog-title"))
            }
      ...
      return@thenApply RateLimitErrorWarningAction(action, content, title, shouldShowUpgradeOption)
    }
  }
}
