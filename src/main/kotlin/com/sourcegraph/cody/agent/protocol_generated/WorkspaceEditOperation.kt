    ...
    val deserializer: JsonDeserializer<WorkspaceEditOperation> =
        JsonDeserializer { element: JsonElement, _: Type, context: JsonDeserializationContext ->
          when (element.getAsJsonObject().get("type").getAsString()) {
            "create-file" ->
                context.deserialize<CreateFileOperation>(element, CreateFileOperation::class.java)
            "rename-file" ->
                context.deserialize<RenameFileOperation>(element, RenameFileOperation::class.java)
            "delete-file" ->
                context.deserialize<DeleteFileOperation>(element, DeleteFileOperation::class.java)
            "edit-file" ->
                context.deserialize<EditFileOperation>(element, EditFileOperation::class.java)
