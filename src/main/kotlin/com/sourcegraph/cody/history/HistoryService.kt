...
class HistoryService(private val project: Project) :
  ...
  fun getDefaultLlm(): LLMState? {
    val account = CodyAccountManager.getInstance(project).account
  ...
  fun getActiveAccountHistory(): AccountData? =
      CodyAccountManager.getInstance(project).account?.let { findEntry(it.id) }

  private fun getOrCreateActiveAccountEntry(): AccountData {
    val activeAccount =
        CodyAccountManager.getInstance(project).account
