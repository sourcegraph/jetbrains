...
class HistoryTree(
  ...
  private val root get() = model.root as RootNode
  private val tree =
  ...
  private fun updatePresentation(chat: ChatState) {
      val leafNotInTree = root.periods()
          .flatMap { it.leafs() }
          .none { it.chat.internalId == chat.internalId }
      if (leafNotInTree) {
  ...
  private companion object {
    ...
    private fun SimpleTree.selectedLeafOrNull() =
        selectionPath?.lastPathComponent as? LeafNode
  }
}
