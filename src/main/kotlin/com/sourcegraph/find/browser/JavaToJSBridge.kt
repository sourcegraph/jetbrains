...
class JavaToJSBridge(private val browser: JBCefBrowserBase) {
  ...
  fun callJS(
      ...
      result: CompletableFuture<JsonObject?>? = null
  ) {
    ...
    Thread {
      ...
      """
                  .trimIndent()
          handler = Function { responseAsString ->
            query.removeHandler(handler!!)
            handler = null
            try {
              val jsonElement = JsonParser.parseString(responseAsString)
              result?.complete(if (jsonElement.isJsonObject) jsonElement.asJsonObject else null)
