...
public class PreviewContent {
  ...
  public static PreviewContent fromJson(Project project, @NotNull JsonObject json) {
    int absoluteOffsetAndLengthsSize =
        isNotNull(json, "absoluteOffsetAndLengths")
            ? json.getAsJsonArray("absoluteOffsetAndLengths").size()
            : 0;
    ...
    return new PreviewContent(
        ...
        Date.from(
            Instant.from(
                DateTimeFormatter.ISO_INSTANT.parse(json.get("timeAsISOString").getAsString()))),
        ...
        isNotNull(json, "symbolContainerName")
            ? json.get("symbolContainerName").getAsString()
            : null,
        isNotNull(json, "commitMessagePreview")
            ? json.get("commitMessagePreview").getAsString()
            : null,
  ...
  public VirtualFile getVirtualFile() {
    if (virtualFile == null) {
      assert fileName
          != null; // We should always have a non-null file name and content when we call
      // getVirtualFile()
      assert content != null;
      virtualFile =
          new SourcegraphVirtualFile(
              fileName, Objects.requireNonNull(getContent()), getRepoUrl(), getCommit(), getPath());
    }
    return virtualFile;
  }
  ...
  private void openInEditor() {
    assert fileName
        != null; // We should always have a non-null file name when we call openInEditor()
    // Open file in editor
