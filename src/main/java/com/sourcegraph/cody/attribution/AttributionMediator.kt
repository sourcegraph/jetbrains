...
/**
 * {@link AttributionMediator} is used to mediate between chat session triggering an attribution
 * search once code snippet was finished printing, and then updating the UI on the code snippet
 * accordingly.
 */
...
class AttributionMediator(private val project: Project) {
  ...
  fun onSnippetFinished(editor: CodeEditorPart, messageId: UUID) {
    if (attributionEnabled()) {
      findChatSessionFor(messageId)
          ?.snippetAttribution(editor.text.get(), callbackInUiThread(editor.attributionListener))
    }
  }
  ...
  /**
   * [AttributionListener] decorator that invokes changes via
  ...
  private fun callbackInUiThread(listener: AttributionListener): AttributionListener =
      AttributionListener { response ->
        ApplicationManager.getApplication().invokeLater { listener.updateAttribution(response) }
      }
}
