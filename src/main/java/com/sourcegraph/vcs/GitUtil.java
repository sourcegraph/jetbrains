...
public class GitUtil {
  ...
  public static String getRemoteRepoUrl(@NotNull GitRepository repository, @NotNull Project project)
      throws Exception {
  ...
  @Nullable
  private static GitRemote getBestRemote(
      @NotNull GitRepository repository, @NotNull Project project) {
  ...
  @Nullable
  private static GitRemote getRemote(
      @NotNull Repository repository, @NotNull Project project, @NotNull String remoteName) {
    GitRepository gitRepository =
        GitRepositoryManager.getInstance(project).getRepositoryForRoot(repository.getRoot());
    if (gitRepository == null) {
      return null;
    }

    List<GitRemote> matchingRemotes =
        gitRepository.getRemotes().stream()
            .filter(x -> x.getName().equals(remoteName))
